...
exports.createMenuMaster = async (req, res) => {
    try {
        const { menuName, menuGroup, menuUrl, sequence, isActive, isParent, parentMenu } = req.body;
...
exports.getMenuMasterById = async (req, res) => {
    try {
        const { menuMasterId } = req.params;
        const menuMaster = await MenuMaster.findById(menuMasterId).populate(
            "menuGroup"
        );
        res.status(200).json({
            isOk: true,
...
exports.getMenuByGroups = async (req, res) => {
    try {
        ...
        const menuGroups = await mongoose.model("MenuGroupMaster").find({ isActive: true }).sort({ sequence: 1 });
