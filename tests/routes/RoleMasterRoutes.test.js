...
jest.mock("../../controllers/RoleMasterController", () => ({
  ...
  listAllRoles: jest.fn((req, res) =>
    res.status(200).json({ message: "All roles" }),
  ),
  ...
  listRoleByParams: jest.fn((req, res) =>
    res.status(200).json({ message: "Roles by params" }),
  ),
}));
...
describe("RoleMaster Routes", () => {
  ...
  it("should list all roles", async () => {
    const res = await request(app).get("/api/auth/list/roles");
  ...
  it("should delete a role", async () => {
    const res = await request(app).delete("/api/auth/delete/roles/1");
    expect(res.statusCode).toEqual(200);
  });

  it("should get a role by id", async () => {
    const res = await request(app).get("/api/auth/get/roles/1");
    expect(res.statusCode).toEqual(200);
  });
