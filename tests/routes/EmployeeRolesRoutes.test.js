...
jest.mock('../../controllers/EmployeeRolesController', () => ({
    createEmployeeRoles: jest.fn((req, res) => res.status(201).json({ message: 'EmployeeRoles created' })),
    getEmployeeRoles: jest.fn((req, res) => res.status(200).json({ message: 'EmployeeRoles found' })),
    updateEmployeeRoles: jest.fn((req, res) => res.status(200).json({ message: 'EmployeeRoles updated' })),
}));
...
describe('EmployeeRoles Routes', () => {
    ...
    it('should get employee roles', async () => {
        const res = await request(app)
            .get('/api/auth/get/employee-roles/1');
