...
jest.mock("../../controllers/LocationController", () => ({
  ...
  listAllCountries: jest.fn((req, res) =>
    res.status(200).json({ message: "All countries" }),
  ),
  deleteCountry: jest.fn((req, res) =>
    res.status(200).json({ message: "Country deleted" }),
  ),
  updateCountry: jest.fn((req, res) =>
    res.status(200).json({ message: "Country updated" }),
  ),
  createState: jest.fn((req, res) =>
    res.status(201).json({ message: "State created" }),
  ),
  listAllStates: jest.fn((req, res) =>
    res.status(200).json({ message: "All states" }),
  ),
  deleteState: jest.fn((req, res) =>
    res.status(200).json({ message: "State deleted" }),
  ),
  updateState: jest.fn((req, res) =>
    res.status(200).json({ message: "State updated" }),
  ),
  listStateByCountry: jest.fn((req, res) =>
    res.status(200).json({ message: "States by country" }),
  ),
  listStateByParams: jest.fn((req, res) =>
    res.status(200).json({ message: "States by params" }),
  ),
  createCity: jest.fn((req, res) =>
    res.status(201).json({ message: "City created" }),
  ),
  listAllCities: jest.fn((req, res) =>
    res.status(200).json({ message: "All cities" }),
  ),
  deleteCity: jest.fn((req, res) =>
    res.status(200).json({ message: "City deleted" }),
  ),
  updateCity: jest.fn((req, res) =>
    res.status(200).json({ message: "City updated" }),
  ),
  listCityByState: jest.fn((req, res) =>
    res.status(200).json({ message: "Cities by state" }),
  ),
  listCityByParams: jest.fn((req, res) =>
    res.status(200).json({ message: "Cities by params" }),
  ),
  listCountryStateCity: jest.fn((req, res) =>
    res.status(200).json({ message: "Country state city" }),
  ),
  getCountryById: jest.fn((req, res) =>
    res.status(200).json({ message: "Country found" }),
  ),
  listCountryByParams: jest.fn((req, res) =>
    res.status(200).json({ message: "Countries by params" }),
  ),
  getStateById: jest.fn((req, res) =>
    res.status(200).json({ message: "State found" }),
  ),
  getCityById: jest.fn((req, res) =>
...
describe("Location Routes", () => {
  ...
  it("should update a country", async () => {
    const res = await request(app)
      .put("/api/auth/update/country/1")
      .send({ name: "test" });
    expect(res.statusCode).toEqual(200);
  });
  it("should list countries by params", async () => {
    const res = await request(app)
      .post("/api/auth/listbyparams/country")
      .send({ name: "test" });
    expect(res.statusCode).toEqual(200);
  });
  ...
  it("should list states by params", async () => {
    const res = await request(app)
      .post("/api/auth/listbyparams/state")
      .send({ name: "test" });
    expect(res.statusCode).toEqual(200);
  });
  ...
  it("should list cities by params", async () => {
    const res = await request(app)
      .post("/api/auth/listbyparams/city")
      .send({ name: "test" });
    expect(res.statusCode).toEqual(200);
  });
